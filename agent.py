...
class player:
    ...
    def help_me(self, unit, pos):
        ...
        action, unit = self.gameBoard.path_finding(
            unit=unit, end_point=pos, bomb=10, help=True)
            ...
            if tot_cost <= bomb:
                ...
                if n.obj_type == 'w':
                    ...
                    if n.id == start.id:
                        ...
                        self.p[unit].next_node(action=2)
                        # return None , unit
                        return action[2], unit
                ...
                print(" CALLING HELP ")
                action, player_unit = self.p[unit].help_me(
                    unit='d', pos=self.p[unit].id)

                print("UNIT ", player_unit, "ACTION ", action)

                return action, player_unit

        except Exception as e:
            print("ERROR IS : ", e)

    def path_finding(self, unit, end_point, bomb=3, help=False):
...
class Agent():
    ...
    async def _on_game_tick(self, tick_number, game_state):
        ...
        for unit_id in my_units:
            ...
            action, unit_id = self.board.path_finding(
                unit=unit_id, end_point=[12, 2], bomb=2)
