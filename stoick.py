...
#######################################################################################################################
    ...
    def coreAtk(self):
        ...
        if self.id in surr or self.id == self.center or self.breakWood:
            ...
            if len(fxf[0]) > 0:
                ...
                self.breakWood = True
                return self.gameBoard.path_finding(unit=self.unit_id, end_point=fxf[1][0])
    ...
    def soldier(self):
        ...
        if self.eInfo['unit'] not in self.gameBoard.eTeam and len(self.gameBoard.eTeam) > 1:
...
class node:
    ...
    def add_neibour(self, grid, evade=False, atk=False, directed=False, diameter=None):
        ...
        if evade and atk:
            ...
            # RIGHT
            if self.col < 14 and grid[self.id + 1].obj_type == '.' and grid[self.id + 1].player == None:
                eNeigh.append(grid[self.id + 1])
            ...
            # while grid[u_id].row > 0 and grid[u_id - 15].obj_type ==  '.' and grid[u_id - 15].player == None and up: #UP
            ...
            # while  grid[d_id].row < 14 and grid[d_id + 15].obj_type ==  '.' and grid[d_id + 15].player == None and down: #DOWN
            ...
            # while  grid[l_id].col > 0 and grid[l_id -1].obj_type ==  '.'  and grid[l_id - 1].player  == None and left: #LEFT
            ...
            # while grid[r_id].col < 14 and grid[r_id + 1].obj_type ==  '.' and grid[r_id + 1].player  == None and right: #RIGHT
            ...
            # UP
            if self.row > 0 and grid[self.id - 15].weight != float("inf"):
                neibours.append(grid[self.id - 15])

            # DOWN
            if self.row < 14 and grid[self.id + 15].weight != float("inf"):
                neibours.append(grid[self.id + 15])

            # LEFT
            if self.col > 0 and grid[self.id - 1].weight != float("inf"):
                neibours.append(grid[self.id - 1])

            # RIGHT
            if self.col < 14 and grid[self.id + 1].weight != float("inf"):
                neibours.append(grid[self.id + 1])
...
class Gameboard:
    ...
    def render(self, game=False, low=False, inf=False, path_trace=False, path=None):
    ...
    def update_board(self, gameState, tick_number=None, call=True):
        ...
        # Team 7 - to find closer unit to center
        if tick_number < 2 and call and self.bara == 1:
            ...
            if len(notAss) == 2:
                ...
                for x in d.keys():
                    ...
                    for ma in self.currAtks.keys():
                        del finalD[ma]
                    print('finalD', finalD)

                    minF = min(finalD.values())
                    print('minF', minF)

                    self.currAtks[d[x][minF]] = x
                    self.p[x].updEn(d[x][minF])
                    self.p[x].setRole('soldier')
    ...
    def bomb_evade(self, unit=None, end_node=None, teamCheck=False) -> int:  # IF RADIUS IS EQUAL TO 2
        ...
        for b in self.bombs:
            ...
            bomb = b
            # if 2*self.h(b , self.p[unit]) -1 < b.blast_diameter:
            if True:
                # check_bomb.append(b)

                print(" CHECK BOMB ", b.id,
                      "BOMB DIAMETER : ", bomb.blast_diameter)
        ...
        # atk = "".join(map(str,self.attack_spot))
        atk = "".join(map(lambda x: str(x.id), self.attack_spot))

        atk_loc = " ".join(map(lambda x: str(x.id), self.attack_spot))

        # print(" TYPE ",type(atk) , atk , atk_loc , "MY LOC : " , start.id)
        print(atk, atk_loc, "MY LOC : ", start.id)

        try:

            safe_spot = self.p[unit].bomb_hash[atk]
            # safe_spot = self.p[unit].bomb_hash.get(atk)
            print(" TRY CHECK THE TYPE ", type(safe_spot))

            # if safe_spot in self.attack_spot:
            #     raise Exception(" RETRACE ")
            ...
            while q:
                ...
                if a in self.attack_spot:
                    ...
                    start_check = True
                    # NEED TO CHECK WETHER ALL ARE INFINITY
                    neibours = a.add_neibour(self.grid, evade=True)
        ...
        if start_check:  # THEN CENTER IS REALLY ONE SAFE SPOT OUT OF ALL
        ...
        if safe_spot in self.attack_spot:  # IF IN CASE NO WAY TO OUT
        ...
        # return safe_spot
        if sp:
            self.p[unit].bomb_hash[atk] = sp.id
            return sp.id
        else:
            self.p[unit].bomb_hash[atk] = sp
            return sp
        ...
        if True:
            if len(path) > 0:
                ...
                if n.obj_type == 'x':
                    ...
                    evade = self.bomb_evade(
                        unit=unit, end_node=self.p[unit].id)
                    ...
                    if evade != None:
                        ...
                        action, unit = self.path_finding(
                            unit=unit, end_point=self.p[unit].goto[-1], check=True)
                    ...
                    if len(path) > 1:
                        ...
                        n2 = path[len(path)-2]
                        # n2 = path[-2]
                        action, unit = self.p[n.player].move_on(
                            start, to_node=n2)
                        return action, unit
                        ...
                        action, unit = self.p[n.player].move_on(
                            start, to_node=None, told_by=n.player, single_Step=True)
                        return action, unit
    ...
    def path_finding(self, unit, end_point, check=False, team7=False):
