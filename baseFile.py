...
class player:
    ...
    def where(self):
        ...
        i = 1
        while signal:
            surr = self.whoseThr(i)
            for x in surr:
                if self.grid[x].around == 0:
                    action = self.gameBoard.path_finding(self.unit_id, x)
                    if action[0] != None:
                        return action
            i += 1

    def takeMove(self):
        if self.grid[self.id].around == 0:
...
class node:
    ...
    def fillAround(self, grid):
        if self.around in ['a', 'b']:
            return
        else:
            self.around = 0
        ...
        if self.weight == 0.1:
            # UP
            if self.row > 0 and grid[self.id - 15].weight != 0.1:
                self.around += 1

            # DOWN
            if self.row < 14 and grid[self.id + 15].weight != 0.1:
                self.around += 1

            # LEFT
            if self.col > 0 and grid[self.id - 1].weight != 0.1:
                self.around += 1

            # RIGHT
            if self.col < 14 and grid[self.id + 1].weight != 0.1:
                self.around += 1

        elif self.weight > 0.1:
            self.around = '-'

        if self.row == 0 and self.weight == 0.1:
            self.around += 1
        if self.col == 0 and self.weight == 0.1:
            self.around += 1
        if self.row == 14 and self.weight == 0.1:
            self.around += 1
        if self.col == 14 and self.weight == 0.1:
            self.around += 1
...
class Gameboard:
    ...
    def render(self):
        ...
        for i in range(225):
            self.grid[i].fillAround(self.grid)
            if i % 15 == 0:
                print()
        ...
        while not open_set.empty():
            ...
            if current == end:
                ...
                action, unit_player = self.take_action(
                    came_from=path, current=current, start=start, end=end, unit=unit, bomb=bomb)
...
class Agent():
    ...
    async def _on_game_tick(self, tick_number, game_state):
        ...
        for unit_id in my_units:
            ...
            # METHOD TO CATCH SPAN
            # action, unit_id = self.board.catch_spans(unit_id)

            # take tactical pos
            action, unit_id = self.board.p[unit_id].takeMove()
